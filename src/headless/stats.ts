...
export interface ModelAgg {
  ...
  avgShotsPerWin: number; stalemateRate: number; ballsPotted: number
  p50: number; p95: number
  inputTokens: number; outputTokens: number; costUsd: number | null   // null = no price entry for this model
}

export interface H2H { wins: number; losses: number; stalemates: number }
...
export function aggregate(games: GameResult[], models: string[]): ModelAgg[] {
  const init = () => ({
    games: 0, wins: 0, losses: 0, stalemates: 0, voids: 0,
    shots: 0, fouls: 0, valid: 0, illegal: 0, apiErrors: 0, balls: 0,
    shotsInWins: 0, lat: [] as number[], inTok: 0, outTok: 0,
  })
  ...
  for (const g of games) {
    for (const side of [0, 1] as const) {
      ...
      const a = acc.get(model); if (!a) continue
      const s = g.stats[side]
      a.games++; a.shots += s.shots; a.fouls += s.fouls
      a.valid += s.validMoves; a.illegal += s.illegalMoves; a.apiErrors += s.apiErrors
      a.balls += s.ballsPotted; a.lat.push(...s.latenciesMs)
      a.inTok += s.inputTokens; a.outTok += s.outputTokens
      if (g.outcome === 'void') a.voids++
      else if (g.winner === null) a.stalemates++
      else if (g.winner === side) { a.wins++; a.shotsInWins += g.totalShots }
      else a.losses++
    }
  }
  ...
  return models.map(model => {
    ...
    return {
      ...
      foulRate: pct(a.fouls, a.shots), apiErrorRate: pct(a.apiErrors, a.shots),
      avgShotsPerWin: a.wins ? a.shotsInWins / a.wins : 0,
      stalemateRate: pct(a.stalemates, a.games), ballsPotted: a.balls,
...
export function headToHead(games: GameResult[], models: string[]): { models: string[]; matrix: H2H[][] } {
  ...
  for (const g of games) {
    const i = idx.get(g.modelA), j = idx.get(g.modelB)
    if (i == null || j == null || i === j) continue
    if (g.outcome === 'void') continue
    if (g.winner === null) { matrix[i][j].stalemates++; matrix[j][i].stalemates++ }
    else if (g.winner === 0) { matrix[i][j].wins++; matrix[j][i].losses++ }
    else { matrix[i][j].losses++; matrix[j][i].wins++ }
  }
  ...
  if (r.headToHead.models.length > 1) {
    ...
    r.headToHead.models.forEach((m, i) => {
      const cells = r.headToHead.models.map((_, j) =>
        i === j ? '—' : `${r.headToHead.matrix[i][j].wins}–${r.headToHead.matrix[i][j].losses}–${r.headToHead.matrix[i][j].stalemates}`)
      o.push(`| **${m}** | ${cells.join(' | ')} |`)
    })
  }

  const warn = r.leaderboard.some(m => m.wilsonHi - m.wilsonLo > 0.30) || r.meta.gamesPerPair < 20
