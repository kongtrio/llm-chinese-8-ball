...
const scripted: MoveSource = async (_model, snap) => {
  ...
  const mine = snap.balls
    .filter((b) => b.num !== 0 && !b.potted)
...
describe('computeShot (shared source of truth)', () => {
  it('full-power straight shot has speed MAX_CUE along +x, no spin', () => {
    ...
    expect(s.wx).toBeCloseTo(0, 10)
    expect(s.wy).toBeCloseTo(0, 10)
    expect(s.wz).toBeCloseTo(0, 10)
  })
...
describe('wilson', () => {
  it('gives sane intervals and clamps the edges', () => {
    const m = wilson(7, 10)
    expect(m.lo).toBeGreaterThan(0.35)
    expect(m.lo).toBeLessThan(0.45)
    expect(m.hi).toBeGreaterThan(0.85)
    expect(m.hi).toBeLessThan(0.92)
...
describe('playGame', () => {
  ...
  it('treats null fields as malformed (not a coerced 0) but still plays', async () => {
    const nullPow: MoveSource = async (m, s) => ({
      move: { ...(await scripted(m, s)).move, power: null as unknown as number },
    })
  ...
  it('accumulates per-call token usage into player stats', async () => {
    const withUsage: MoveSource = async (m, s) => ({
      ...(await scripted(m, s)),
  ...
  it('can play bot-basic without API keys or a move source', async () => {
    const r = await playGame('bot-basic', 'bot-basic', 0, { anthropic: '', openai: '' }, { maxShots: 6 })
    expect(r.totalShots).toBeGreaterThan(0)
    expect(r.errors).toHaveLength(0)
    expect(r.stats[0].apiErrors + r.stats[1].apiErrors).toBe(0)
    expect(r.stats[0].inputTokens + r.stats[1].inputTokens).toBe(0)
  })
})

describe('runBenchmark resume', () => {
  it('skips completed games loaded from the existing checkpoint/report', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'pool-bench-'))
    const out = join(dir, 'run.json')
    const log = console.log
    console.log = () => {}
    try {
      await runBenchmark(['A', 'B'], KEYS, { gamesPerPair: 2, concurrency: 1, out, maxShots: 2 }, scripted)
      let calls = 0
      await runBenchmark(
        ['A', 'B'],
        KEYS,
        { gamesPerPair: 2, concurrency: 1, out, maxShots: 2, resume: true },
        async (m, s) => {
          calls++
          return scripted(m, s)
        },
...
describe('pricing', () => {
  it('prices known models and returns null for unknown ones', () => {
    ...
    expect(costUsd('bot-basic', 1_000_000, 1_000_000)).toBe(0)
