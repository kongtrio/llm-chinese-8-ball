...
export function rack(): Ball[] {
  ...
  for (let i = 0; i < 7; i++) {
    order.push(solids[i])
    order.push(stripes[i])
  }
  order.splice(4, 0, 8)
  order.length = 15 // 8 at the rack centre
  const footX = L * 0.72,
    dx = BR * Math.sqrt(3),
    gap = 2 * BR + 0.0003
  let k = 0
  for (let row = 0; row < 5; row++)
    for (let i = 0; i <= row; i++) balls.push(mkBall(order[k++], footX + row * dx, W / 2 + (i - row / 2) * gap))
  return balls
}
...
export function freeSpot(balls: Ball[], x: number, y: number): { x: number; y: number } {
  ...
  for (let rad = 2 * BR; rad < L; rad += BR)
    for (let a = 0; a < 360; a += 20) {
      const nx = x + Math.cos((a * Math.PI) / 180) * rad,
        ny = y + Math.sin((a * Math.PI) / 180) * rad
