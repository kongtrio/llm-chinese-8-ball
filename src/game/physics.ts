...
export function integrate(b: Ball, dt: number) {
  const u = slip(b),
    s = Math.hypot(u.x, u.y)
  if (s > SLIP_EPS) {
    // sliding: kinetic friction opposes slip
    const fx = (-MU_SLIDE * G * u.x) / s,
      fy = (-MU_SLIDE * G * u.y) / s
    b.vx += fx * dt
    b.vy += fy * dt
    ...
    // rolling: rolling resistance opposes v
    const v = Math.hypot(b.vx, b.vy)
    if (v > 0) {
      const nv = Math.max(0, v - MU_ROLL * G * dt)
      b.vx *= nv / v
      b.vy *= nv / v
    }
    b.wy = b.vx / BR
    b.wx = -b.vy / BR // enforce pure roll
  }
...
function cushion(b: Ball, ctx: ShotCtx) {
  ...
  if (b.x < BR) {
    b.x = BR
    b.vx = -b.vx * E_CUSH
    b.vy += RAIL_SPIN * BR * b.wz
    b.wz *= RAIL_SPIN_DECAY
    hit = true
  } else if (b.x > L - BR) {
    b.x = L - BR
    b.vx = -b.vx * E_CUSH
...
function substep(balls: Ball[], dt: number, ctx: ShotCtx) {
  ...
  for (const b of balls) {
    ...
    for (const p of POCKETS)
      if (Math.hypot(b.x - p.x, b.y - p.y) < p.r) {
        b.potted = true
        b.vx = b.vy = b.wx = b.wy = b.wz = 0
        if (b.num === 0) ctx.cuePotted = true
        else ctx.potted.push(b.num)
        break
      }
  }
  ...
  for (let i = 0; i < balls.length; i++) {
    const a = balls[i]
    if (!onTable(a)) continue
    for (let j = i + 1; j < balls.length; j++) {
      const b = balls[j]
      if (onTable(b)) collide(a, b, ctx)
    }
  }
}
