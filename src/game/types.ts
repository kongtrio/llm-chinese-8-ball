...
export interface Ball {
  ...
  y: number // position (m)
  vx: number
  vy: number // linear velocity (m/s)
  wx: number
  wy: number
  wz: number // angular velocity (rad/s); wz = vertical (side English)
  rx: number
  ry: number
  rz: number // visual orientation (rad)
  potted: boolean
}
...
export type PlayerType = 'human' | 'llm' | 'bot'
export interface PlayerConfig {
  type: PlayerType
  model: string
}
export interface RuntimePlayer extends PlayerConfig {
  group: Group | null
  name: string
}
