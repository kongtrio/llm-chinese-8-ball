...
export const provider = (model: string): 'anthropic' | 'openai' => (model.startsWith('claude') ? 'anthropic' : 'openai')
  ...
  input_schema: {
    ...
    properties: {
      reasoning: {
        type: 'string',
        description:
          'Your plan: which target ball and pocket; why both the cue→ball and ball→pocket lines are clear of other balls; the power/spin choice; where you expect the cue ball to end up; and the next ball you intend to play.',
      },
      angle_degrees: {
        type: 'number',
        description:
          'Cue-ball travel direction. 0=right(+x), 90=down(+y), 180=left, 270=up. Clockwise (y points down).',
      },
...
export function buildPrompt(s: Snapshot): string {
  ...
  o.push(
    'You are a world-class Chinese 8-ball player. Play the highest-percentage shot, control the cue ball for your next shot, plan the whole run-out — and above all, DO NOT FOUL.',
  )
  o.push(
    `Table play area ${cm(L)}x${cm(W)} cm. Origin top-left, +x = right, +y = DOWN, so angle_degrees is clockwise (0=right, 90=down, 180=left, 270=up). Ball diameter ${dia} cm.`,
  )
  o.push(
    'Pockets (x,y cm): ' +
      POCKETS.map((p) => `(${cm(p.x)},${cm(p.y)})`).join(' ') +
      '. Pockets are TIGHT — aim precisely.',
  )
  o.push(
    'Your group: ' +
      (s.group
        ? `${s.group}s (solids = 1-7, stripes = 9-15; the 8-ball is neither)`
        : s.isBreak
          ? 'BREAK — table open'
          : 'OPEN TABLE — the first ball you legally pot sets your group'),
  )
  if (s.group)
    o.push(
      `Win by potting all your ${s.group}s, THEN legally potting the 8. Potting the 8 before your group is cleared, or fouling as you pot it, LOSES the game.`,
    )
  o.push(
    s.ballInHand
      ? 'You have BALL-IN-HAND: place the cue anywhere via cue_x, cue_y (cm) — put it where you have the easiest, straightest pot.'
      : `Cue ball at (${cm(s.cue.x)}, ${cm(s.cue.y)}) cm.`,
  )
  ...
  if (s.isBreak) {
    o.push(
      'THIS IS THE BREAK — aim straight at the apex (front) rack ball nearest the cue and hit near full power (0.92–1.0) to scatter the pack. Add a little draw/stun (spin_y −0.2..0) so the cue stays mid-table and does not scratch. Do not play it soft.',
    )
  } else {
    o.push(
      [
...
async function callClaude(model: string, key: string, prompt: string): Promise<MoveResult> {
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model,
      max_tokens: 8096,
      tools: [TOOL], // Anthropic requires a cap; generous headroom
  ...
  if (!call) {
    const why =
      data.status === 'incomplete'
        ? `incomplete (${data.incomplete_details?.reason})`
        : `status=${data.status ?? 'unknown'}`
    throw new Error(`no tool call — ${why}`)
  }
  let move: Move
  try {
    move = JSON.parse(call.arguments) as Move
  } catch {
...
async function callChat(model: string, key: string, prompt: string): Promise<MoveResult> {
  ...
  let move: Move
  try {
    move = JSON.parse(call.function.arguments) as Move
  } catch {
    throw new Error('tool call arguments were not valid JSON')
  }
  return {
